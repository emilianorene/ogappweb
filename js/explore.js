...
var tipo_transaccion = ["No Definido","Alquiler","Venta"];
 $(document).ready(function () {
	...
	$("#latitude").val(getParameterByName("lat"));
	$("#longitude").val(getParameterByName("lng")); 	
	traerDatos(getParameterByName("lat"),
               getParameterByName("lng"),   
               getParameterByName("operacion"),
               null,
               getParameterByName("habitaciones"),
               getParameterByName("banos"),
               getParameterByName("precioDesde"),
               getParameterByName("precioHasta"),
               null,
               null);
	cargarSliders();
...
function traerDatos(lat, lng,operacion, tipoInmueble, habitaciones, banos, precioDesde, precioHasta, areaDesde, areaHasta)
{
    props = []; 
	$.ajax(
	{
...
{
	var map_lat = -25.291714;
	var map_lng = -57.612168;
	
	var lat_comp = lat;
	var lng_comp = lng;
	
	if( typeof lat_comp != "undefined"  && typeof lng_comp != "undefined" && lat_comp !=""  && lng_comp != "")
	{
		map_lat = lat_comp;
		map_lng = lng_comp;
	}
...
$('#btn_filtrar').click(function () {
    var prices = $('.priceSlider').slider("option", "values");
    var areas = $('.priceSlider').slider("option", "values");
	traerDatos($("#latitude").val(),
               $("#longitude").val(),
               $('input[name=tType]:checked').val(),
               $('input[name=pType]:checked').val(),
               $('#bedrooms').val(),
               $('#bathrooms').val(),
               prices[0],
               prices[1],
	...
	/*window.location.href = "explore.html?operacion="+$('input[name=tType]:checked').val()+
										'&tipoInmueble='+$('input[name=pType]:checked').val()+
										'&habitaciones='+$('#bedrooms').val()+
										'&banos='+$('#bathrooms').val()+
										'&precioDesde='+prices[0]+
										'&precioHasta='+prices[1]+ 
                                        '&areaDesde='+areas[0]+
                                        '&areaHasta='+areas[1];*/
});	
    ...
    if(e.which == 13) {
		...
		geocoder.geocode( { 'address': address}, function(results, status) {
		  ...
		  if (status == google.maps.GeocoderStatus.OK) {
            ...
            traerDatos($("#latitude").val(),$("#longitude").val(),null,null,null,null,null,null,null,null);		
		});     
